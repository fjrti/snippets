 ...
 *
 * Compilation :
 *    gcc -O2 -o inject inject.c -lm          # par d�faut
 *    gcc -O2 -DNO_SD -o inject inject.c      # d�sactive sdht (pas besoin de libm)
 *    gcc -O2 -DDONT_ANALYSE_HTTP_HEADERS -o inject inject.c -lm  # d�sactive parseur HTTP
 *
 ...
 * 2000/11/22 : limitation du nombre de sockets simultan�es par client.
 ...
 *              TODO: g�rer le referer, et controler la longueur des cookies lors des remplacements.
 * 2001/03/02 : ajout de headers personnalis�s
 ...
 * 2001/08/09 : Affinage du delai d'affichage des stats � la milliseconde. Ce
 *              temps devient param�trable par l'option "-w".
 *              Le d�marrage en douceur fonctionne bien, m�me pour des petits d�lais.
 *              Le temps cumul� affich� est d�sormais "-1" tant que l'injection n'a pas
 *              pu d�marrer.
 ...
 *              gestion m�moire pour tenter de faire tenir plus d'utilisateurs.
 *		Il faudrait refaire la partie serveur (voir haproxy) pour lib�rer les headers
 *		aussit�t que possible, et lire les data � la vol�e.
 * 2004/05/14 : ajout de l'option '-a' pour afficher la date absolue.
 * 2004/09/26 : passage en mode multi-processus
 * 2005/01/10 : affichage des moyenne et ecart-type des temps de r�ponse
 * 2005/01/19 : Ne plus d�marrer un client si pas assez de FD.
 *		Synth�se des statistiques avant affichage final.
 * 2005/08/09 : ajout des options -H, -T, -G, -P pour faire le sc�nario en ligne de commande.
 * 2005/10/09 : correction du nom 'host' si utilisation de -G.
 *
 * Remarque : le champ "variables" HTTP peut contenir 2 "%s" qui seront remplac�s par l'id du client
 ...
 *       pas. C'est maintenant rattrap� par le timeout. Sans doute une table
 *       de sockets qui se fige quelque part, ou un bug sur l'OS :-(
 *       Voir la trace � la fin de ce fichier.
 *
 * TODO :
 *  - r��crire en g�rant des files d'attente suivant l'�tat des clients.
 *  - g�rer les unit�s d'affichage
 *
 * D�tail des affichages :
 *  time delta clients    hits ^hits hits/s  ^h/s     bytes   ^bytes  kB/s  last  errs  tout htime ptime
 *
 * - time est le nombre de millisecondes �coul�es depuis le d�but du test
 * - delta est le temps en millisecondes depuis le dernier affichage
 * - clients est le nombre total de clients ayant d�roul� un sc�nario (avec ou sans erreurs)
 * - hits est le nombre total de hits r�ussis (=sans erreur) depuis le d�but du test
 * - ^hits est le nombre de hits r�ussis depuis le dernier affichage
 * - hits/s est le nombre moyen de hits par seconde depuis le d�but du test
 * - ^h/s est le nombre moyen de hits par seconde depuis le dernier affichage
 * - bytes est le nombre total d'octets lus depuis le d�but du test
 * - ^bytes est le nombre d'octets recus depuis le dernier affichage
 * - kB/s est le d�bit moyen depuis le d�but du test, en kilooctets par seconde
 * - ^k/s est le d�bit moyen depuis le dernier affichage, en ko/s
 * - errs est le nombre d'erreurs (d�connexions ...) depuis le d�but du test
 * - tout est le nombre de timeouts depuis le d�but du test
 * - htime donne l'�volution du temps moyen d'un hit (en ms)
 * - ptime donne l'�volution du temps moyen de chargement d'une page avec tous ses objets (en ms)
 *
 */
...
struct stats {
    ...
    double tot_htime, tot_sqhtime;	/* utilis�s pour le calcul de l'�cart-type */
} *stats = NULL;
...
/* retourne un pointeur sur une structure de type sockaddr_in remplie d'apr�s
   la chaine <str> entr�e au format "123.123.123.123:12345". */
...
/* cr�e un objet pour une page :
   ...
   Toutes les chaines sont r�allou�es dynamiquement (strdup).
*/   
...
/* cr�e une page pour un client existant. Les username et password du client doivent exister */
...
/* tente de rajouter les fetchs non initi�s pour une page donn�e.
   renvoie 0 si le client a �t� supprim� suite � une erreur.
   renvoie 1 si le fetch a correctement �t� initi�, et 2 s'il a �chou� (manque de ressources)
*/
/*static inline*/ int continue_fetch(struct page *page) {
	...
	if (unlikely(nbconn+arg_maxobj >= arg_maxsock || page->actobj >= arg_maxobj)) {
	    /* on ne peut pas d�marrer le fetch tout de suite */
	...
	if (unlikely((fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) == -1)) {
	    //	    fprintf(stderr, "impossible de cr�er une socket, destruction du client.\n");
	...
	if (unlikely((connect(fd, (struct sockaddr *)&obj->addr, sizeof(obj->addr)) == -1) && (errno != EINPROGRESS))) {
	    if (errno == EAGAIN || errno == EADDRNOTAVAIL) { /* plus de ports en local -> r�essayer plus tard */
		put_free_port(page->client->addr, obj->local_port, thr-1);
		obj->local_port = 0;
		close(fd);
		if (attempts++ < 10)
		...
		close(fd);
		return 2;
		/* ne pas renouveler le client, sinon on risque une r�cursivit� ! */
		//destroyclient(page->client, NULL); /* renvoie un pointeur sur le suivant */
		//return 0; /* killed client */
	    }
...
/* d�marre un client. Si une erreur se produit lors du d�marrage, alors
   les ressources sont lib�r�es et le client est d�truit.
   retourne 0 si le client a �t� d�truit.
*/
int start_client(struct client *cli) {
    ...
    //SETNOW(&page->begin);
    /* le client est (re)d�marr� */
    return 1;
}


/* retourne 0 si le client a du �tre d�truit. */
...
static inline int onemoreclient() {
    ...
    /* on entre dans le scheduler directement � partir de ce client */
...
/* detruit le client, lib�re ses fd encore ouverts, d�compte le nombre de connexions
   associ�es, et d�cr�mente le nombre de clients actifs */
void destroyclient(struct client *client, struct client *prev) {
	...
	if ((prev != NULL) &&
	    (prev->next == client)) { /* si on connait le pr�c�dent dans la liste, on y va directement */
	}
	else {
	    prev = clients;
	...
	if ((arg_maxiter > 0) && (stats[thr].iterations >= arg_maxiter)) {
	    ...
	    break; /* si ret==0, plus possible d'injecter � cause d'erreurs */
	}
    }
...
int show_stats(void *arg) {
	...
	if (tv_cmp(&now, &nextevt) < 0)
	    ...
	    //if (totalhits > 0)
		if ((starttime.tv_sec | starttime.tv_usec) == 0)  /* la premiere fois, on d�marre le chrono */
		    SETNOW(&starttime);
...
/* ce scheduler s'occupe d'injecter des clients tant qu'il n'y en a pas assez, mais jamais
   ...
   calcule une prochaine date 'next' servant de r�f�rence pour le prochain ajout de clients.
   Les clients sont ajout�s d�s que (now-next) > arg_slowstart.
*/
...
/*
 * ce scheduler s'occupe de g�rer les clients (think time, ...)
 */
int scheduler(void *arg) {
	...
	if (cli->status == STATUS_RUN) {
	    ...
	    if (cli->current->objecttostart != NULL) { /* s'il reste des fetches � effectuer, on les fait. */
		if (continue_fetch(cli->current) == 0)
			continue; /* le client a �t� d�truit ? */
	    }
	    prev = cli; /* conserve le chainage pour acc�l�rer la suppression */
	    continue;
	}
	else if (cli->status == STATUS_THINK) {
	    unsigned int del2;
	    if ((del2 = tv_remain(&now, &cli->nextevent)) > 0) {  /* attendre avant de passer � la page suivante */
		delay = MINTIME(del2, delay);
		prev = cli; /* conserve le chainage pour acc�l�rer la suppression */
		continue;
	    }
...
#endif
		if ((cli->pages = cli->pages->next) == NULL || stopnow) { /* fin du sc�nario pour ce client */
...
void SelectRun() {
  ...
  while (!stopnow) {
	  ...
	  if (nbactcli < arg_nbclients && !stopnow) {
	      /* ne pas y aller si ce n'est pas n�cessaire */
...
#endif
      /* On va appeler le select(). Si le temps fix� est nul, on consid�re que
	 c'est un temps infini donc on passe NULL � select() au lieu de {0,0}.  */
      status=select(maxfd,
...
int EventRead(int fd) {
	...
	if (obj->buf == NULL) { /* pas encore allou� */
	    obj->read = obj->buf = (char *)alloc_pool(buffer);
	}
...
#endif
	{
...
#endif
		{
			...
			ret = recv(fd, NULL, INT_MAX, MSG_NOSIGNAL | MSG_TRUNC);  /* d�truire les data sans les stocker */
#else
	...
	if (ret > 0) {
	    ...
	    /* on rel�ve les mesures le plus t�t possible */
	    ...
	    ptr1 = obj->buf;
	    while (ptr1 < obj->read) {
		/* si on a un saut de ligne ici, c'est forc�ment une fin de headers */
		if ((*ptr1 == '\r') || (*ptr1 == '\n'))
		    break;
		...
		if ((ptr1-ptr2 > 11) && !strncasecmp(ptr2,"Set-Cookie:",11)) {
		    ...
		    /* recherche du d�but de la partie utile du cookie */
		    ptr2+=11;
		    while ((ptr2 < ptr1) && isspace(*ptr2))
			ptr2++;

		    /* on va se d�barrasser des "path=" */
		    //		    *ptr1=0;
		    ...
		    if (ptr3 <= ptr1-5) {
			/* et on se d�barrasse aussi de "; path=" */
		    ...
		    /* on ne devrait pas faire ceci car on �crase peut-etre un CR ou un LF */
		    //		    *ptr3=0;


		    /* a ce niveau, le nouveau cookie est compris exactement entre ptr2 et ptr3
		     * sous la forme var=val sans d�limiteur. ptr2 est termin� par un zero.
		     */
		    //		    fprintf(stderr,"---ptr2-ptr3=<%s>\n", ptr2);

		    /* le cookie est compris entre ptr2 et ptr3 */
		    /* on va le concat�ner au cookie existant */
	    ...
	    // Log(LOG_LDEBUG,"[Event] erreur, arr�t du client <%p>.\n", obj->page->client);
...
void usage() {
    fprintf(stderr,
	    ...
	    "- scnfile  : nom du fichier de sc�nario a utiliser\n"
...
int main(int argc, char **argv) {
    ...
    *p = '\0';
    ...
    //fprintf(stderr, "Fin de lecture du sc�nario.\n");
 ...
 * --> Voir la socket 4 : elle ne se d�bloque jamais.
 * rem: un probl�me similaire a �t� observ� sur sizesrv dans la gestion des signaux.
 *
